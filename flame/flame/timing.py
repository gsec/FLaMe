...
def flake_growth(flake, itersize=1000):
    """ This is a wrapper method which we can handle in Flake instances. Each of those
...
def snow_storm(flake_count, parallel=False, **kwargs):
    ...
    # cloud = ((flakes, itersize) for flakes in cloud)
    logger.info("Generate cloud of size [{}]: parallel={}".format(flake_count, parallel))
    start = arrow.now()
    # logger.info("@{}:: parallel={} ".format(start, parallel))
    if parallel:
        with Pool() as p:
